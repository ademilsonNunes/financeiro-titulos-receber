...
export interface TimelineRecebimentoItem {
  empresa?: string;
  nf?: string;
  parcela?: string;
  ...
  aging?: { [key: string]: any };
  [key: string]: any;
}

@Injectable({ providedIn: 'root' })
export class TimelineRecebimentoService {
  private http = inject(HttpClient);
  private appConfig = inject(AppConfigService);
  private base = '';

  constructor() {
    this.base = `${this.getApiBasePath()}/v1/timeline-recebimento`;
  }
  ...
  list(paramsObj: Record<string, any> = {}): Observable<TimelineRecebimentoItem[]> {
    ...
    if (!(environment as any).production) {
      try {
        ...
        console.debug('[TimelineRecebimentoService] GET', this.base, { params: p });
      } catch {}
    }
      ...
      .pipe(
        map((resp) => {
          ...
          try {
            ...
            console.error('Falha ao parsear resposta da API timeline-recebimento:', {
              status: resp.status,
          ...
          return arr as TimelineRecebimentoItem[];
        })
      );
  }
}