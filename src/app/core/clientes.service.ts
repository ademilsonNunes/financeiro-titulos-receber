...
export class ClientesService {
  ...
  constructor() {
    this.base = `${this.getApiBasePath()}/v1/clientes`;
  }
  ...
  list(paramsObj: Record<string, any> = {}): Observable<ClienteDTO[]> {
      ...
      .pipe(
        map((resp) => {
          ...
          try {
            ...
            console.error('Falha ao parsear resposta da API de clientes:', {
              status: resp.status,
          ...
          return arr.map((c: any) => ({
            codigo: c.codigo ?? c.codigoCliente ?? c.id ?? '',
            nome: c.nome ?? c.nomeCliente ?? c.razaoSocial ?? c.fantasia ?? '',
            cnpj: c.cnpj ?? c.cpfCnpj ?? c.documento ?? '',
            loja: c.loja ?? '',
